...
def test_plan_sequence_through_registry(registry, sequence_context) -> None:
    plan = registry.plan(
        TechniqueRequest(
...
def test_plan_logit_through_registry(registry, logit_context) -> None:
    plan = registry.plan(
        TechniqueRequest(
...
def test_external_plan_yields_channel_not_import(
    ...
    assert plan.external_execution.import_forbidden is True
    assert plan.external_execution.enable_network_isolation is True
    assert plan.channel_contract is not None
    assert plan.channel_contract["plan_filename"] == "technique_plan.json"
